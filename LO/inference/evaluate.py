...
def showPlotsRow(shot, complexity):
    ...
    G, L, M, T = np.ones((shot,complexity)), np.ones((shot,complexity)), np.ones((shot,complexity)), np.ones((shot,complexity))
    for i in range(shot):
...
def showPlotsCol(shot, complexity):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('./OP/LOAlongColumn.png')
    plt.show()