...
class LinearOrderTask(Task):
    def __init__(self, tokenizer: PreTrainedTokenizer, num_examples: int, no_prompts: int,
                 bin: bool = False, complexity: int = None):
        super().__init__(tokenizer)
        if not bin:
            file = os.path.join(config.DATA_DIR, 'linearorder', "LinearOrderDec_50.json")
        else:
            file = os.path.join(config.DATA_DIR, 'linearorder', "LinearOrderBin_50.json")
        with open(file) as f:
