...
filename = 'DIV_30.json'
if not os.path.exists('OP'):
    os.makedirs('OP')

def produce_response(model_name, Input, p):
    if model_name == "gemma2" or "llama3":
        response = ollama.chat(model=model_name, messages=[{'role': 'user', 'content': Input}, {'role': 'system',
                                                                                                'content': 'Answer in specified Format. Do not output explanation.'}],
                               stream=True, options={'seed': 0, 'temperature': 0.5, 'raw': True, 'num_ctx': 4096})
    else:
        response = ollama.chat(model=model_name, messages=[{'role': 'user', 'content': Input}], stream=True,
                               options={'seed': 0, 'temperature': 0.5, 'raw': True, 'num_ctx': 4096})
    ...
    for model_name in models:
        ...
        for cnt, p in enumerate(problems['enumeration']):
            ...
            if 'answer' not in problems[p].keys():
                problems[p]['answer'] = {}
            if cnt == 4932 and model_name == 'mathstral':
                problems[p]['answer'][model_name] = ''
                continue
            problems[p]['answer'][model_name] = produce_response(model_name, Input, p)
            # print('\n'+ '=='*20 + 'Was performing: ' + model_name + " ___ " + p  + '=='*20)
            sys.stdout.write(
                "\r----Model: %s Performing: %s after (%0.2f)sec. \t Model Running: (%d)min \t Total Time: (%d)min or (%d)days-----" % (
            ...
            if int(p.split('_')[1]) % 25 == 0 or p == problems['enumeration'][-1]:
                ...
                with open('./OP/'+filename[:-5] + '_op' + '.ckpt', 'a') as f:
                    f.write(
                        f"Model: %s Saved: %s after (%0.2f)sec. \t Model Running: (%d)min \t Total Time: (%.2f)min\n" % (
                        model_name, p, (time.time() - st_p), (time.time() - st_m) // 60, (time.time() - st) // 60))
            time.sleep(0.2)
